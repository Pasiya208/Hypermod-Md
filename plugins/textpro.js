...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command }) => {
  if (!effect) throw 'යොදන්න .textpro <effect>\n*TEXT TO IMG:*\n.textpro\n\n╭─❑ 「 LIST EFFECT 」 ❑──\n' + effects.map(v => v.title).join('\n│ • ')
  effect = effect.toLowerCase()
  if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw ` *${effect}* ඔබ යොදා ගත් කමාන්ඩ් එක වැරදිය නැවත උත්සහ කරන්න`
  let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
  if (text.includes(split)) text = text.split(split)
  text = Array.isArray(text) ? text : [text]
  let res = await textpro(effect, ...text)
  if (typeof res == 'number') throw res == -1 ? ` *${effect}* නැවත උත්සහ කරන්න` : `මෙසේ යොදන්න ${usedPrefix}${command} ${effect} ${new Array(res).fill('text').map((v, i) => v + (i ? i + 1 : '')).join('|')}`
  let result = await axios.get(res, {
    responseType: 'arraybuffer'
  })
  await m.reply('*කරුණාකර පොම්ඩක් ඉම්න!*')
