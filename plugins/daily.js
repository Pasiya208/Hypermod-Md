...
let handler = async (m, { conn, usedPrefix, isPrems }) => {
  if (db.data.users[m.sender].level < 1) throw `ඔබට ටයිප් කිරීම ඇසෙන මට්ටම ඉහළට\n${usedPrefix}levelup`
  let time = db.data.users[m.sender].lastclaim + 86400000
  if (new Date - db.data.users[m.sender].lastclaim < 86400000) throw `ඔබ අද දෛනික හිමිකම් පෑමට හිමිකම් පා ඇත\nබලා සිටින්න ${msToTime(time - new Date())} නැවතත්`
  db.data.users[m.sender].exp += isPrems ? prem * db.data.users[m.sender].level : free * db.data.users[m.sender].level
  m.reply(`+${isPrems ? prem * db.data.users[m.sender].level : free * db.data.users[m.sender].level} XP\n\nඉහළ මට්ටම, ඔබට ලැබෙන XP වැඩි වේ`)
