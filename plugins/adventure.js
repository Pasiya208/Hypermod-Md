...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let str = `
Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Japan', 'Korea', 'Bali', 'America', 'Iraq', 'Arabic', 'Pakistan', 'German', 'Finland', 'Bring to the dream world', 'The end of the world', 'Mars' , 'Moon', 'Pluto', 'Sun', 'His heart', '...'])} dan mendapatkan
*exp:* ${exp} 
*money:* ${uang}
*rubbish:* ${sampah}${potion == 0 ? '' : '\n*Potion:* ' + potion + ''}${diamond == 0 ? '' : '\n*diamond:* ' + diamond + ''}${common == 0 ? '' : '\n*common crate:* ' + common + ''}${uncommon == 0 ? '' : '\n*uncommon crate:* ' + uncommon + ''}
`.trim()
            conn.reply(m.chat, str, m)
            if (mythic > 0) {
                   global.db.data.users[m.sender].mythic += mythic * 1
                   conn.reply(m.chat, '*සුභ පැතුම්, ඔබට දුර්ලභ අයිතමයක් ලැබී ඇත, එනම්*\n' + mythic + ' Mythic Crate', m)
            }
            if (legendary > 0) {
                global.db.data.users[m.sender].legendary += legendary * 1
                conn.reply(m.chat, '*ඔබට එපික් අයිතමය ලැබුනා යැයි සුභ පැතුම්*\n' + legendary + ' Legendary Crate', m)
            }
            ...
            } else conn.reply(m.chat, `ඔබ වික්‍රමාන්විත හා වෙහෙසට පත්ව ඇත, කරුණාකර උත්සාහ කරන්න *${timers}* නැවතත්`, m)
        } else conn.reply(m.chat, 'ත්‍රාසජනක ගමනක් යාමට හැකි වීමට අවම සෞඛ්‍ය 80 ක්, ටයිප් කිරීමෙන් පළමුව ජීවිතයක් මිලදී ගන්න *' + usedPrefix + 'shop buy potion <amount>*\nand type *' + usedPrefix + 'use potion <amount>*\n\n_නොමිලේ මුදල් සහ ඖෂධ ලබා ගැනීමට වර්ගය_ *' + usedPrefix + 'collect*', m)
    } catch (e) {
        console.log(e)
